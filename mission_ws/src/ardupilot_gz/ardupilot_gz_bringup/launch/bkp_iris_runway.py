...
def generate_launch_description():
    ...
    # Caminho para seu mundo personalizado
    custom_world_path = Path(pkg_project_gazebo) / "worlds" / "teste_mundo.sdf"
    ...
    rviz = Node(
        package="rviz2",
        executable="rviz2",
        arguments=["-d", str(Path(pkg_project_bringup) / "rviz" / "iris.rviz")],
        condition=IfCondition(LaunchConfiguration("rviz")),
    )
